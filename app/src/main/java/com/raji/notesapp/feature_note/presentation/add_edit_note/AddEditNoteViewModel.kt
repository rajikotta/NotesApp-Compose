...
class AddEditNoteViewModel @Inject constructor(private val noteUseCases: NoteUseCases,
    ...
    private val _noteTitle = mutableStateOf(NoteTextFieldState(hintText = "Enter Title"))
    val noteTitle: State<NoteTextFieldState> = _noteTitle

    private val _noteContent = mutableStateOf(NoteTextFieldState(hintText = "Enter some content"))
        ...
        savedStateHandle.get<Int>("noteId")?.let { noteId ->
            if (noteId != -1) {
                viewModelScope.launch {
                    noteUseCases.getNoteDetailsUsecase(noteId)?.also { note ->
